...

{
  ...
  check('209 school cards', d.querySelectorAll('#schoolList .card').length === 209);
  ...
  privCb.click();
  check('recheck private restores schools', d.querySelectorAll('#schoolList .card').length === 209);
  ...
  check('academics is real data for tested public HS', det.querySelector('.whit[data-gkey="stem"]').getAttribute('data-v') !== 'na');
  dom.window.renderDetail('school', 'Kent Denver');
  check('academics n/a for private (no state tests)', det.querySelector('.whit[data-gkey="stem"]').getAttribute('data-v') === 'na');
  check('outcome vitals render on school cards (grad + proficiency)', /Graduation rate/.test(d.getElementById('schoolList').textContent) && /proficient/.test(d.getElementById('schoolList').textContent));
  check('real outcome data present in source', (html.match(/,out:\{/g)||[]).length >= 140);
...
}
{
  ...
  await tick(400);
  const d = dom.window.document;
  check('all 209 schools carry program grades', (html.match(/,g:\{exp:/g) || []).length === 209);
  check('all 209 schools carry vitals', (html.match(/,st:\{enroll:/g) || []).length === 209);
  check('all 209 schools carry diversity data', (html.match(/,dv:\{soc:/g) || []).length === 209);
  ...
  check('every fit grade is derived from its ratio', fitBad === 0 && fitOK === 209);
  ...
  check('all 209 schools carry about paragraphs', (html.match(/,about:\['/g) || []).length === 209);
