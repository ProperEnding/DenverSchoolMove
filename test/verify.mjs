...

{
  ...
  check('92 school cards', d.querySelectorAll('#schoolList .card').length === 92);
  ...
  privCb.click();
  check('recheck private restores schools', d.querySelectorAll('#schoolList .card').length === 92);
...
}
{
  ...
  await tick(400);
  const d = dom.window.document;
  check('all 92 schools carry program grades', (html.match(/,g:\{exp:/g) || []).length === 92);
  check('all 92 schools carry vitals', (html.match(/,st:\{enroll:/g) || []).length === 92);
  d.querySelector('.seg-btn[data-lvl="ECE"]').click();
  check('ECE level filter shows the 30-school band', d.querySelectorAll('#schoolList .card').length === 30 && /Boulder Journey/.test(d.querySelector('#schoolList .card .name').textContent));
  ...
  check('all 92 schools carry about paragraphs', (html.match(/,about:\['/g) || []).length === 92);
